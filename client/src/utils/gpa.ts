...
export const calculateGPA = (courses: Course[]): number => {
  ...
  const hasCredits = courses.some(course => course.credits && course.credits > 0);
...
export const calculateCumulativeGPA = (semesters: Array<{ gpa: number; courses: Course[] }>): number => {
